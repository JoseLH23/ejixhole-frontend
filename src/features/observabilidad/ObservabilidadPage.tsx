...
function Estado({ valor }: { valor: string }) {
  const operativo = valor === "healthy" || valor === "up";
  const pendiente = valor === "unknown";
  const texto = operativo ? "Operativo" : pendiente ? "Sin medición" : valor === "degraded" ? "Degradado" : "No disponible";
  const clase = operativo
    ...
    <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div><p className="font-medium">{nombre}</p><p className="text-xs text-muted-foreground">{detalle}</p></div>
        </div>
        <Estado valor={estado} />
      </div>
    </div>
...
function porcentaje(valor: number | null | undefined) {
  return valor == null ? "Pendiente" : `${valor.toFixed(3)}%`;
}
    ...
    <div className="space-y-4">
      ...
      {isLoading ? <div className="h-32 animate-pulse rounded-xl bg-muted" /> : <>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <Componente nombre="Cola durable" detalle={jobs ? `${jobs.pending ?? 0} pendientes · ${jobs.running ?? 0} ejecutándose` : "Sin respuesta"} estado={jobs?.status ?? "unavailable"} icon={ListTodo} />
        </div>

        <div className="grid gap-4 lg:grid-cols-2">
          <section className="rounded-xl border border-border bg-card p-4 shadow-sm">
            <div className="mb-3 flex items-center justify-between"><h2 className="font-semibold">SLO del Backend</h2><Estado valor={backend?.http.slo.status ?? "unavailable"} /></div>
            <div className="grid gap-2 sm:grid-cols-3">
              <Metrica etiqueta="Disponibilidad" valor={porcentaje(backend?.http.slo.availability_percent)} objetivo={backend ? `${backend.http.slo.targets.availability_percent}%` : undefined} />
              <Metrica etiqueta="Errores 5xx" valor={porcentaje(backend?.http.slo.error_rate_percent)} objetivo={backend ? `≤ ${backend.http.slo.targets.error_rate_percent}%` : undefined} />
              <Metrica etiqueta="Latencia p95" valor={backend ? `${backend.http.latency_ms.p95} ms` : "Pendiente"} objetivo={backend ? `≤ ${backend.http.slo.targets.latency_p95_ms} ms` : undefined} />
            </div>
          </section>
          <section className="rounded-xl border border-border bg-card p-4 shadow-sm">
            <div className="mb-3 flex items-center justify-between"><h2 className="font-semibold">SLO de MH-Core</h2><Estado valor={mhCore?.slo.status ?? "unavailable"} /></div>
            <div className="grid gap-2 sm:grid-cols-3">
              <Metrica etiqueta="Disponibilidad" valor={porcentaje(mhCore?.slo.availability_percent)} objetivo={mhCore ? `${mhCore.slo.target_percent}%` : undefined} />
              <Metrica etiqueta="Tiempo conocido" valor={mhCore ? `${Math.round(mhCore.slo.known_seconds)} s` : "Pendiente"} />
              <Metrica etiqueta="Dead-letter" valor={deadLetters == null ? "Desconocido" : String(deadLetters)} objetivo="0" />
            </div>
          </section>
        </div>

...
import { Pencil, Percent, Plus, Power, Trash2, X } from "lucide-react";
...
export function TarifasEspecialesPage() {
  ...
  function abrirEditar(tarifa: TarifaEspecial) {
    ...
    setForm({
      ...
      porcentaje_ajuste: Number(tarifa.porcentaje_ajuste),
      aplica_a: tarifa.aplica_a,
      dias_semana: tarifa.dias_semana ? [...tarifa.dias_semana] : null,
      prioridad: tarifa.prioridad,
      unidad_hospedaje_id: tarifa.unidad_hospedaje_id,
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div><p className="text-xs font-semibold uppercase tracking-wider text-primary">Ingresos</p><h1 className="font-display text-2xl font-semibold">Tarifas especiales</h1><p className="text-sm text-muted-foreground">Temporadas, promociones y ajustes automáticos por fecha.</p></div>
        <button type="button" onClick={abrirNueva} className="inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground"><Plus className="h-4 w-4" /> Nueva tarifa</button>
      </div>
            ...
            return <article key={tarifa.id} className={`rounded-2xl border bg-card p-4 shadow-sm ${!tarifa.activa ? "opacity-60" : ""}`}>
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex gap-1">
                  <button type="button" title="Editar" onClick={() => abrirEditar(tarifa)} className="rounded-lg p-2 text-muted-foreground hover:bg-muted hover:text-foreground"><Pencil className="h-4 w-4" /></button>
              ...
              <div className="mt-3 grid grid-cols-2 gap-2 text-xs"><div className="rounded-lg bg-muted/50 p-2"><span className="text-muted-foreground">Aplica a</span><p className="font-medium">{APLICA[tarifa.aplica_a]}</p></div><div className="rounded-lg bg-muted/50 p-2"><span className="text-muted-foreground">Prioridad</span><p className="font-medium">{tarifa.prioridad}</p></div><div className="rounded-lg bg-muted/50 p-2"><span className="text-muted-foreground">Días</span><p className="font-medium">{tarifa.dias_semana?.map((dia) => DIAS[dia]).join(", ") || "Todos"}</p></div><div className="rounded-lg bg-muted/50 p-2"><span className="text-muted-foreground">Unidad</span><p className="truncate font-medium">{nombreUnidad(tarifa.unidad_hospedaje_id) || "Todas"}</p></div></div>
      ...
      {abierto && <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/40 p-4 backdrop-blur-sm"><div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl border bg-card shadow-xl">
        <div className="flex items-start justify-between border-b p-5"><div><h2 className="font-display text-xl font-semibold">{editandoId ? "Editar tarifa especial" : "Nueva tarifa especial"}</h2><p className="text-sm text-muted-foreground">Configura temporada, promoción o fin de semana.</p></div><button type="button" onClick={() => setAbierto(false)} className="rounded-lg p-2 hover:bg-muted"><X className="h-4 w-4" /></button></div>
        <form onSubmit={guardar} className="space-y-4 p-5">
          ...
          <div className="flex justify-end gap-2"><button type="button" onClick={() => setAbierto(false)} className="rounded-lg border px-4 py-2 text-sm">Cancelar</button><button type="submit" disabled={guardando} className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground disabled:opacity-50">{guardando ? "Guardando…" : editandoId ? "Guardar cambios" : "Guardar tarifa"}</button></div>
        </form>
      </div></div>}
    </div>
  );

...
export function DashboardPage() {
  ...
  if (!data || data.tarjetas.length === 0) {
    return <EmptyState titulo="Todavía no hay datos" icon={Inbox} descripcion="Cuando empieces a registrar clientes, reservaciones y pagos, el resumen aparecerá aquí." />;
  }
  ...
  const fechaFormateada = (() => {
    try { return format(new Date(`${data.fecha}T00:00:00`), "EEEE d 'de' MMMM 'de' yyyy", { locale: es }); }
  ...
  if (kpisRow.length > 0) {
    widgets.push({
      id: "indicadores-principales", titulo: "Indicadores principales",
      descripcion: "Resumen rápido de clientes, reservaciones, ingresos y caja.", categoria: "Dirección",
      tamanosPermitidos: ["grande"], tamanoInicial: "grande",
      contenido: <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2 lg:grid-cols-4">{kpisRow.map((tarjeta) => {
  ...
  widgets.push({
    id: "estado-operativo", titulo: "Estado operativo",
    descripcion: "Estado del sistema, resumen mensual y servicios con mayor movimiento.", categoria: "Operación",
    tamanosPermitidos: ["grande"], tamanoInicial: "grande",
    contenido: <div className="grid grid-cols-1 gap-3 lg:grid-cols-3"><DashboardSidePanel ultimaActualizacion={dataUpdatedAt} /><ResumenMensualCard tarjeta={ingresosDelMes} />{esAdmin ? <ServiciosMasVendidosPanel /> : <div className="hidden lg:block" />}</div>,
  });
  ...
  if (esAdmin) {
    widgets.push(
      ...
      },
      {
        ...
        descripcion: "Llegadas confirmadas agrupadas por día para preparar la operación.", categoria: "Calendario",
        tamanosPermitidos: ["mediano", "grande"], tamanoInicial: "mediano", contenido: <ProximasReservacionesCards />,
      },
      {
        id: "actividad-reciente", titulo: "Actividad reciente",
        descripcion: "Últimos movimientos importantes registrados en el sistema.", categoria: "Operación",
        tamanosPermitidos: ["mediano", "grande"], tamanoInicial: "mediano", contenido: <ActividadReciente />,
      },
      {
        id: "reservaciones-por-estado", titulo: "Reservaciones por estado",
        descripcion: "Distribución actual de reservaciones según su estado.", categoria: "Reportes",
        tamanosPermitidos: ["mediano", "grande"], tamanoInicial: "mediano", contenido: <ReservacionesPorEstadoChart />,
      },
      {
        id: "ingresos-ultimos-dias", titulo: "Ingresos de los últimos 7 días",
        descripcion: "Tendencia reciente de ingresos para detectar cambios rápidamente.", categoria: "Finanzas",
        tamanosPermitidos: ["mediano", "grande"], tamanoInicial: "mediano", contenido: <IngresosUltimos7DiasChart />,
      }
    );
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
        <span className="inline-flex items-center gap-1.5 rounded-full bg-success/10 px-3 py-1 text-xs font-medium text-success"><Circle className="h-2 w-2 fill-current" />Sistema operativo</span>
      ...
      {!esAdmin && <p className={cn("text-center text-xs text-muted-foreground")}>Algunas secciones (actividad, agenda operativa y reportes) requieren rol de administrador.</p>}
    </div>
  );
}

...
const resultado = (value?: string | null) => value === "helped" ? "Ayudó" : value === "neutral" ? "Neutral" : value === "not_helpful" ? "No ayudó" : null;
...
export function MhPredictionsPanel() {
  ...
  const decision = useMutation({
    mutationFn: ({ businessDate, code, value }: { businessDate: string; code: string; value: "accepted" | "dismissed" }) => dashboardApi.saveRecommendationDecision(businessDate, code, value),
    onSuccess: () => client.invalidateQueries({ queryKey: ["dashboard", "mh-core", "predictions"] }),
  });
  const outcome = useMutation({
    mutationFn: ({ businessDate, code, value }: { businessDate: string; code: string; value: "helped" | "neutral" | "not_helpful" }) => dashboardApi.saveRecommendationOutcome(businessDate, code, value),
    onSuccess: () => client.invalidateQueries({ queryKey: ["dashboard", "mh-core", "predictions"] }),
  });
  ...
  return <div className="space-y-3">
    ...
    <Card><CardHeader className="pb-2"><CardTitle className="text-base">Recomendaciones medibles</CardTitle></CardHeader><CardContent className="space-y-2">{data.recommendations.map((item) => <div key={item.code} className="rounded-lg border bg-muted/40 p-3 text-sm"><p>{item.message}</p><div className="mt-2 flex flex-wrap gap-2"><Button size="sm" variant="outline" disabled={decision.isPending || outcome.isPending} onClick={() => decision.mutate({ businessDate: data.business_date, code: item.code, value: "accepted" })}>Aplicar</Button><Button size="sm" variant="ghost" disabled={decision.isPending || outcome.isPending} onClick={() => decision.mutate({ businessDate: data.business_date, code: item.code, value: "dismissed" })}>Descartar</Button>{item.decision?.decision === "accepted" && !item.decision.outcome && <><Button size="sm" variant="outline" disabled={outcome.isPending} onClick={() => outcome.mutate({ businessDate: data.business_date, code: item.code, value: "helped" })}>Ayudó</Button><Button size="sm" variant="outline" disabled={outcome.isPending} onClick={() => outcome.mutate({ businessDate: data.business_date, code: item.code, value: "neutral" })}>Neutral</Button><Button size="sm" variant="outline" disabled={outcome.isPending} onClick={() => outcome.mutate({ businessDate: data.business_date, code: item.code, value: "not_helpful" })}>No ayudó</Button></>}{item.decision && <span className="self-center text-xs text-muted-foreground">Decisión: {item.decision.decision}{resultado(item.decision.outcome) ? ` · Resultado: ${resultado(item.decision.outcome)}` : ""}</span>}</div></div>)}</CardContent></Card></div>
  </div>;
}
